...
imds = imageDatastore('./Single','labelsource', 'foldername');
fds = fileDatastore("./SingleLabel",'ReadFcn',@parseLabel);
i = 1;
while hasdata(imds) && hasdata(fds)
    label = read(fds);
    imds.Labels(i) = label(9);
    i = i+1;
end
[imdsTrain,imdsValidation] = splitEachLabel(imds,0.7); %%training70% validation30%
...
% a download link. 
% 
% To try a different pretrained network, open this example in MATLAB? and 
% select a different network. For example, you can try |squeezenet|, a network 
% that is even faster than |googlenet|. You can run this example with other pretrained 
...
%% 
% Find the names of the two layers to replace. You can do this manually 
% or you can use the supporting function <matlab:edit(fullfile(matlabroot,'examples','nnet','main','findLayersToReplace.m')) 
...
% Replace the classification layer with a new one without class labels. |trainNetwork| 
...
% layers to freeze. In GoogLeNet, the first 10 layers make out the initial 'stem' 
% of the network. Use the supporting function <matlab:edit(fullfile(matlabroot,'examples','nnet','main','freezeWeights.m')) 
...
% In the previous step, you increased the learning rate factors for the last learnable 
